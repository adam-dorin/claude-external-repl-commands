...
function start(passthroughArgs = []) {
  ...
  let savedUmask; // POSIX: restrict the socket's birth mode, restored after bind
  ...
  function bindNow() {
    if (!isWin) {
      ...
      // Create the socket owner-only from birth (no brief world-accessible window);
      // chmod in onListen tightens it to 0600. Restored once bound.
      try {
